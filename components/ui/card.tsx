...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'bg-card text-card-foreground flex flex-col gap-4 sm:gap-6 rounded-lg sm:rounded-xl border py-4 sm:py-6 shadow-xs sm:shadow-sm',
        className,
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('text-lg sm:text-xl leading-none font-semibold', className)}
      {...props}
    />
  )
}
