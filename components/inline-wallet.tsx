...
export function InlineWallet({ mode = 'collector', connected = true }: InlineWalletProps) {
  ...
  const handleWithdrawPi = () => {
    ...
    setWithdrawAmount('')
    setWithdrawAddress('')
    alert(`Withdrawal request for ${withdrawAmount} ${PI} submitted`)
  }
                ...
                {user.piaddr && (
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <Button variant="ghost" size="sm" onClick={copyAddress} className="h-6 w-6 p-0">
                      {copied ? <Check className="w-3 h-3 text-green-500" /> : <Copy className="w-3 h-3" />}</Button>
                  </div>
                )}
              </div>
            </div>
          ...
          <Card className="h-[calc(100vh-280px)] flex flex-col">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg flex items-center gap-2">
                <span className="text-[#7B3FE4] text-xl font-bold">{PI}</span>
                Platform Balance
              </CardTitle>
              <CardDescription className="text-sm">Your internal {PI} balance on Aurafloor</CardDescription>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col overflow-hidden">
              <div className="text-center py-6 flex-shrink-0 bg-gradient-to-b from-[#7B3FE4]/5 to-transparent rounded-xl mb-4">
                <div className="text-4xl font-bold text-[#7B3FE4]">
                  {PI} {platformBalance.toFixed(2)}
                </div>
              ...
              <div className="space-y-4 flex-1 overflow-y-auto pt-4">
                <div className="space-y-3">
                  <Label htmlFor="deposit-amount" className="text-sm flex items-center gap-1">
                    <Plus className="w-3 h-3 text-green-500" /> Add {PI} to Platform
                  </Label>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#7B3FE4] font-bold">{PI}</span>
                      <Input
                        id="deposit-amount"
                        placeholder="0.00"
                        type="number"
                    ...
                    <Button
                      ...
                      className="gap-2 bg-gradient-to-r from-[#7B3FE4] to-[#A855F7] hover:from-[#6B2FD4] hover:to-[#9845E7]"
                    >
                      {isProcessingPayment ? (
                ...
                <div className="space-y-3">
                  <Label htmlFor="withdraw-amount" className="text-sm flex items-center gap-1">
                    <Minus className="w-3 h-3 text-red-500" /> Withdraw {PI}
                  </Label>
                  <div className="flex flex-col gap-2">
                    ...
                    <Button
                      ...
                      disabled={!withdrawAmount || parseFloat(withdrawAmount) <= 0 || parseFloat(withdrawAmount) > platformBalance || !withdrawAddress}
                ...
                <div className="space-y-3">
                  {ownedNFTs.map((nft) => (
                    <div key={nft.tokenId} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-accent/5 transition-colors">
                      ...
                      >
                        <div className="w-full h-full bg-gradient-to-br from-[#7B3FE4]/20 to-[#A855F7]/20 rounded-lg flex items-center justify-center">
                          <Music className="w-6 h-6 text-[#7B3FE4]/50" />
                        </div>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate">
                          Price: {(nft.price / 1000000).toFixed(2)} {PI}
                        </p>
                    ...
                    <div key={tx.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-accent/5 transition-colors">
                      ...
                      <div className="text-right flex-shrink-0 ml-4">
                        <div className={`font-medium text-sm ${tx.amount > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {tx.amount > 0 ? '+' : ''}{tx.amount.toFixed(2)} {PI}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <Card className="border-t rounded-t-none">
              <CardContent className="p-4">
                ...
                <div className="text-xs space-y-1 text-muted-foreground">
                  <p>&bull; Uses Pi Network&apos;s official payment system</p>
                  <p>&bull; All transactions recorded on Pi blockchain</p>
                  <p>&bull; We don&apos;t hold private keys</p>
                  <p>&bull; Withdrawals require verification</p>
                </div>
          ...
          {selectedNFT && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 border rounded-lg">
                <div className="w-12 h-12 bg-gradient-to-br from-[#7B3FE4]/20 to-[#A855F7]/20 rounded-lg flex items-center justify-center">
                  <Music className="w-6 h-6 text-[#7B3FE4]/50" />
                </div>
                <div>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-[#7B3FE4] to-[#A855F7]"
                >
                  {isListing ? (
