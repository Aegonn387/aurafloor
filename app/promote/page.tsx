...
// ------------------------------------------------------------------
// Types based on your database (table `n` and `pc`)
// ------------------------------------------------------------------
interface NFT {
  id: string
  title: string
  crid: string      // creator user id
...
export default function PromotePage() {
  ...
  // Explicitly typed – NEVER becomes `never[]`
  ...
  // Fetch NFTs owned/created by this user from your `n` table
  useEffect(() => {
    const fetchNFTs = async () => {
      try {
        ...
        toast({
          title: "Error",
          description: "Could not load your NFTs. Please refresh.",
          variant: "destructive",
        })
      } finally {
  ...
  const estimatedCost = dailyBudget * days
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!selectedTrack) throw new Error("Selected NFT not found")
      ...
      toast({
        ...
        description: `Your promotion for ${selectedTrack.title} is now pending payment confirmation`,
      })
        ...
        <main className="container px-4 py-12">
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                You haven't minted any NFTs yet. Create your first NFT to start promoting.
              </p>
              <Button onClick={() => router.push("/mint")}>Mint Your First NFT</Button>
            </CardContent>
          </Card>
        </main>
        <MobileNav />
                ...
                <Card className="bg-muted/50">
                  <CardContent className="p-4">
                    <div className="flex gap-4">
                      <img
                        src={creatorNFTs.find((n) => n.id === selectedNFT)?.cimg || "/placeholder.svg"}
                        alt="NFT Cover"
                        className="w-20 h-20 rounded-lg object-cover"
                      />
                      <div className="flex-1">
                        <p className="font-semibold">{creatorNFTs.find((n) => n.id === selectedNFT)?.title}</p>
                        <p className="text-sm text-muted-foreground">
                          {creatorNFTs.find((n) => n.id === selectedNFT)?.genre}
                        </p>
                        <Badge variant="secondary" className="mt-2">
                          {creatorNFTs.find((n) => n.id === selectedNFT)?.price}π
                        </Badge>
                      </div>
                    </div>
                  </CardContent>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Add a custom message to highlight your NFT..."
                  rows={3}
                />
              </div>
