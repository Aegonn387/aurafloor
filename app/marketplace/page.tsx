...
import { Loader2, Search, Filter, Music, User, Play, Headphones, Flame, CheckCircle, Mic2, Radio, Drum, Volume2, Layers, Headset } from 'lucide-react'
...
interface NFT {
  ...
  metadata?: { name?: string; description?: string; image?: string; duration?: number; category?: string }
...
const CATEGORIES = [
  'All', 'Music', 'Podcast', 'Beats', 'Sound Effects', 'Samples', 'Loops', 'Voiceovers', 'Electronic', 'Hip Hop', 'Acoustic', 'Jazz', 'EDM', 'Other'
]
...
export default function MarketplacePage() {
      ...
      if (listing && listing.active === true) {
        const nft: NFT = {
          ...
          metadata: {
            ...
            description: "NFT listed on marketplace",
  ...
  const filteredNFTs = allNfts.filter(nft => {
    ...
    return matchesSearch && nft.metadata?.category === filter
  })
      ...
      <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Discover, stream, and collect exclusive NFTs</p>
          </div>
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            <div className="flex-1 relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4" />
              <Input placeholder="Search NFTs..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
            <div className="flex gap-4">
              <Select value={filter} onValueChange={setFilter}>
                <SelectTrigger className="w-[160px]"><Filter className="h-4 w-4 mr-2" /><SelectValue placeholder="Filter" /></SelectTrigger>
                <SelectContent>
                  {CATEGORIES.map(cat => (
          ...
          {loading ? (
            ...
            <>
              <div className="mb-4">
                <p className="text-sm text-muted-foreground">{filteredNFTs.length} NFTs</p>
              </div>
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {displayedNFTs.map((nft) => {
                      const CategoryIcon = CATEGORY_ICONS[nft.metadata?.category || 'Other'] || Music
                      return (
                          ...
                          <Card className="overflow-hidden border-border h-full flex flex-col">
                            ...
                            <CardFooter className="p-3 pt-0">
                              <div className="flex gap-1 w-full">
                                <Button size="sm" variant="outline" className="flex-1" onClick={() => playFullStream(nft)}>
                                  <Headphones className="h-3.5 w-3.5 mr-1" />Stream
                                </Button>
